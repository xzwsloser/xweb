    ...
    : deleted_(false),channel_(channel)
{
    ...
    // the time is 0 - 9999
    expired_time_ = (((now_time.tv_sec % 10000) * 1000) + (now_time.tv_usec / 1000)) + timeout;
}
...
TimerNode::TimerNode(TimerNode& tn)
    : channel_(tn.channel_),expired_time_(0) {}
...
bool TimerNode::isVaild()
{
    struct timeval now_time;
    gettimeofday(&now_time, nullptr);
    size_t temp_time = (now_time.tv_sec % 10000) * 1000 + (now_time.tv_usec / 1000); 
